...
def _dec_depth(d: dict):
    from emboviz.core.observations import DepthMap
...
def decode_hidden_states(d: dict):
    from emboviz.core.types import HiddenStates
    return HiddenStates(
        states=d["states"],
        query_position=int(d["query_position"]),
...
def decode_ffn_activations(d: dict):
    from emboviz.core.types import FFNActivations
...
def decode_required_inputs(d: dict):
    from emboviz.models.protocol import RequiredInputs
